...
// regular expression for santity checking the format of an email address.
...
// Define a New function to initialise a custom Form struct. Notice that
...
// MinLength method checks that a specific field in the form contains a
// minimum number of characters. If the check fails then add the
// appropriate message to the form errors.
...
// InList checks whether the provided value is in the given list of
// strings.
func (f *Form) InWordList(field string, list []string) {
	...
	f.Errors.Add(field, "Not a valid word.")
}
...
// PermittedValues checks that a specific field in the form matches one of
// a set of specific permitted values. If the check fails then add the
// appropriate message to the form errors.
